...
export function ProductCard({ product }: ProductCardProps) {
  ...
  return (
    <Link href={`/product/${product.slug}`} className="flex flex-col h-full">
      <Card className="group overflow-hidden border-border/50 transition-all duration-300 hover:border-primary/30 hover:shadow-lg flex flex-col flex-1">
        ...
        <CardContent className="p-3 sm:p-4 flex-1 flex flex-col">
          <div className="mb-2 flex flex-col gap-1">
            <h3 className="font-serif text-base sm:text-lg font-semibold text-foreground line-clamp-2">{product.name}</h3>
            <span className="font-bold text-primary text-lg" suppressHydrationWarning>{formatPrice(product.price)}</span>
          </div>
